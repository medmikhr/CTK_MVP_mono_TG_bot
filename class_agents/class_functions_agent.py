...
def get_agent_instance() -> 'GigaChatFunctionsAgent':
    """Получение или создание экземпляра агента (синглтон)."""
    global _agent_instance
    if _agent_instance is None:
        _agent_instance = GigaChatFunctionsAgent()
...
Всегда используй соответствующие функции для поиска актуальной информации из документов. Дай подробный, структурированный ответ на русском языке."""),
            ...
            while True:
                response = self.agent.invoke({"messages": messages}, config={"configurable": {"thread_id": thread_id}})
                ...
                if "tool_calls" in response and response["tool_calls"]:
                    ...
                    for tool_call in response["tool_calls"]:
                        ...
                        if func_name == "dama_search":
                            ...
                            result = self.dama_search_func.invoke(args)
                        elif func_name == "ctk_search":
                            used_tools.append("Регламенты и материалы ЦТК")
                            result = self.ctk_search_func.invoke(args)
                        else:
                            result = None
