...
def load_document(file_path: str) -> List[Dict]:
    ...
    try:
        if file_extension == '.pdf':
            ...
            loader = None
            # Сначала пробуем автоматически определить кодировку
            try:
                with open(file_path, 'rb') as f:
                    raw_data = f.read()
                    detected = chardet.detect(raw_data)
                    if detected['confidence'] > 0.7:  # Если уверенность высокая
                        encoding = detected['encoding']
                        loader = TextLoader(file_path, encoding=encoding)
                        test_docs = loader.load()
                        if test_docs:
                            logger.info(f"Автоматически определена кодировка {encoding} (уверенность: {detected['confidence']:.2f}): {file_path}")
            except Exception as e:
                logger.debug(f"Автоматическое определение кодировки не удалось: {e}")
            
            # Если автоматическое определение не сработало, пробуем известные кодировки
            if not loader:
                ...
                for encoding in encodings:
                    try:
                        ...
                        test_docs = loader.load()
                        if test_docs:
                            logger.info(f"Успешно загружен TXT файл с кодировкой {encoding}: {file_path}")
                            break
                    except Exception as e:
            ...
            if not loader:
                logger.error(f"Не удалось загрузить TXT файл ни с одной кодировкой: {file_path}")
                return []
        else:
...
def delete_document(document_id: str, collection: str) -> bool:
    ...
    try:
        ...
        if not collection_data or not collection_data['documents']:
            logger.warning(f"Коллекция {collection} пуста")
            return False
        
        # Находим индексы документов, которые нужно удалить
        indices_to_delete = []
        for i, metadata in enumerate(collection_data['metadatas']):
            if metadata.get('source') == document_id:
                indices_to_delete.append(i)
        
        if not indices_to_delete:
            logger.warning(f"Документ {document_id} не найден в коллекции {collection}")
            return False
        
        # Удаляем документы по индексам
        vectorstore._collection.delete(ids=[collection_data['ids'][i] for i in indices_to_delete])
        
        logger.info(f"Документ успешно удален: {document_id} (удалено {len(indices_to_delete)} чанков)")
        return True
