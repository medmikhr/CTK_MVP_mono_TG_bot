...
# Конфигурация коллекций: {collection_id: {"users": [user_ids], "display_name": "Отображаемое имя"}}
COLLECTIONS_CONFIG = {
    "sbf_meta": {
        "users": [135727236],  # Замените на реальные ID пользователей СБФ
        "display_name": "СБФ"
    },
    "ctk_methodology": {
        "users": [673473862],  # Замените на реальные ID пользователей ЦТК
        "display_name": "ЦТК"
    }
}
...
def get_collection_display_name(collection: str) -> str:
    """Возвращает отображаемое имя коллекции"""
    return COLLECTIONS_CONFIG.get(collection, {}).get("display_name", collection.upper())
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_collection:
        ...
        collection_display_name = get_collection_display_name(user_collection)
...
async def load_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    collection_display_name = get_collection_display_name(collection)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверяем, не ожидаем ли мы выбор документа для удаления
        ...
        response_text = call_agent(text, str(user_id))
        end_time = time.time()
        ...
        # Отправляем ответ
        response_with_time = f"{response_text}\n\n⏱️ Время обработки: {processing_time:.2f}с"
        await update.message.reply_text(response_with_time)
